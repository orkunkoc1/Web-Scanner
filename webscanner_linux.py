...
def send_email(results):
    # Email configuration
    sender_email = '_sendersmail_'  # Sender's email address
    receiver_email = '_receivermail_'  # Recipient's email address
    ...
    dotenv_path = Path('_yourpath_/dot.env')
    ...
    smtp_server = '_smtpserver_'  # SMTP server address
    smtp_port = 25  # SMTP server port
    smtp_port = int(smtp_port)
    smtp_username = '_your_username_'  # SMTP username
    smtp_password = os.getenv(key)    # Read your password from this file
...
def perform_search():
    # Using Turkish characters directly with f-string
    keywords = ["kurtlar","vadisi"]  # Modify with your desired keywords
    query = "+".join(keywords)
